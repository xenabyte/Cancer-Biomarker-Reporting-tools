    ...
    <section>
      <div id="exportContent" class="container">
        ...
        <div style="margin-bottom: 100px;" class="col-md-12">
            @if($procedure != '')
                <p><strong>Procedure:</strong><span>@if($procedure == 'Other (specify):') {{ $procedure }} ({{ $procedure_other }}) @else {{ $procedure }} @endif</span></p>
            @endif
            @if($tumorsite_ph != '' || $tumorsite_up != '' || $tumorsite_pb != '' || $tumorsite_pt != '' || $tumorsite_cd != '' || $tumorsite_ns != '' || $tumorsite_ot != '' )
                <p><strong>Tumor Site:</strong><span>
                @if($tumorsite_ph != '') {{ $tumorsite_ph }}, @endif
                @if($tumorsite_up != '') {{ $tumorsite_up }}, @endif
                @if($tumorsite_pb != '') {{ $tumorsite_pb }}, @endif
                @if($tumorsite_pt != '') {{ $tumorsite_pt }}, @endif
                @if($tumorsite_cd != '') {{ $tumorsite_cd }}, @endif
                @if($tumorsite_ns != '') {{ $tumorsite_ns }}, @endif
                @if($tumorsite_ot != '') {{ $tumorsite_ot }} (@if($tumorsite_ot_specify != '') {{ $tumorsite_ot_specify }}). @endif @endif
                </span></p>
            @endif
            @if($tumorsize_cd != '' || $tumorsize_gd != '')
            <p><strong>Tumor Size:</strong>@if($tumorsize_cd != '') Cannot be determined (explain): {{ $tumorsize_cd }} @else  Greatest dimension (centimeters): {{ $tumorsize_gd }}cm @if($tumorsize_length != '' && $tumorsize_width != '') => Additional dimensions (centimeters): {{ $tumorsize_length }}cm X {{ $tumorsize_width }}cm  @endif  @endif<span></span></p>
            @endif
            @if($histologic != '')
            ...
            @endif
            @if($tumor_extension_ne != '' || $tumor_extension_nc != '' || $tumor_extension_tp != '' || $tumor_extension_to != '' || $tumor_extension_td != '' || $tumor_extension_ts != '' || $tumor_extension_ta != '' || $tumor_extension_tm != '' || $tumor_extension_tip != '' || $tumor_extension_tia != '' || $tumor_extension_tv != '' || $tumor_extension_ca != '' )
                <p><strong>Tumor Extension:</strong><span>
                @if($tumor_extension_ne != '') {{ $tumor_extension_ne }}, @endif
                @if($tumor_extension_nc != '') {{ $tumor_extension_nc }}, @endif
                @if($tumor_extension_tp != '') {{ $tumor_extension_tp }}, @endif
                @if($tumor_extension_to != '') {{ $tumor_extension_to }}, @endif
                @if($tumor_extension_td != '') {{ $tumor_extension_td }}, @endif
                @if($tumor_extension_ts != '') {{ $tumor_extension_ts }}
                @if($tumor_extension_ts_tir != '') ({{ $tumor_extension_ts_tir }}), @endif
                @if($tumor_extension_ts_tim != '') ({{ $tumor_extension_ts_tim }}), @endif
                @if($tumor_extension_ts_tie != '') ({{ $tumor_extension_ts_tie }}), @endif
                @if($tumor_extension_ts_tip != '') ({{ $tumor_extension_ts_tip }}). @if($tumor_extension_ts_tip_specify != '') Specify: {{ $tumor_extension_ts_tip_specify }}, @endif @endif
                @endif
                @if($tumor_extension_ta != '') {{ $tumor_extension_ta }}, @endif
                @if($tumor_extension_tm != '') {{ $tumor_extension_tm }}, @endif
                @if($tumor_extension_tip != '') {{ $tumor_extension_tip }}, @endif
                @if($tumor_extension_tia != '') {{ $tumor_extension_tia }}, @endif
                @if($tumor_extension_tv != '') {{ $tumor_extension_tv }}, @endif
                @if($tumor_extension_ca != '') {{ $tumor_extension_ca }}. @endif
                </span></p>
            @endif
            @if($icm != '')
                <p><strong>Margins: </strong><span> {{ $icm }} </span></p>
            @endif
            @if($margin_examined != '')
                <p><strong>Margins:</strong><span>{{ $margin_examined }} </span></p>
            @endif
            @if($icm_distance != '')
                <p><strong>Distance of invasive carcinoma from margin(millimeters):</strong><span>{{ $icm_distance }} </span></p>
            @endif
            @if($icm_closest_margin != '')
                <p><strong>Specify closest margin:</strong><span>{{ $icm_closest_margin }} </span></p>
            @endif
            @if($icm_tumor_distance != '')
                <p><strong>Distance of invasive carcinoma from closest margin(millimeters):</strong><span>{{ $icm_tumor_distance }}mm </span></p>
            @endif
            @if($icm_pppm != '' || $icm_dppm != '' || $icm_om != '')
            ...
            @if($icm_pppm != '')
                ...
                @if($icm_pppm == 'Uninvolved by invasive carcinoma and pancreatic high-grade intraepithelial neoplasia')
                {{ $icm_pppm }} @if($icm_pppm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_pppm_distance }}mm)  @endif
                @elseif($icm_pppm == 'Uninvolved by invasive carcinoma')
                {{ $icm_pppm }} @if($icm_pppm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_pppm_distance }}mm)  @endif
                @else
                {{ $icm_pppm }}
                @endif
            ...
            @if($icm_dppm != '')
                ...
                @if($icm_dppm == 'Uninvolved by invasive carcinoma and pancreatic high-grade intraepithelial neoplasia')
                {{ $icm_dppm }} @if($icm_dppm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_dppm_distance }}mm)  @endif
                @elseif($icm_dppm == 'Uninvolved by invasive carcinoma')
                {{ $icm_dppm }} @if($icm_dppm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_dppm_distance }}mm)  @endif
                @else
                {{ $icm_dppm }}
                @endif
            ...
            @if($icm_om_specify != '')
                ...
                @if($icm_om_specify != '') {{ $icm_om_specify }}  ({{ $icm_om }})   @endif
                </span></p>
            @endif
            @if($icm_euppm != '' || $icm_euom != '')
            <h5><em><u>For enucleation specimens only: </u></em></h5>
            @endif
            @if($icm_euppm != '')
                <p><strong>Pancreatic Parenchymal Margin:</strong><span>
                @if($icm_euppm == 'Uninvolved by tumor') {{ $icm_euppm }} @if($icm_euppm_distance != '') => (Distance of tumor from margin (mm): {{ $icm_euppm_distance }}mm)  @endif
                @else
                {{ $icm_euppm }}
                @endif
                </span></p>
            ...
            @if($icm_euom_specify != '')
                <p><strong>Other Margin(s): </strong><span>
                @if($icm_euom_specify != '')  {{ $icm_euom_specify }}  ({{ $icm_euom }})   @endif
                </span></p>
            @endif
            @if($icm_nppm != '' || $icm_prupm != '' || $icm_bdm != '' || $icm_pmg != '' || $icm_dmj != '')
            <h5><em><u>For pancreaticoduodenal resection specimens only:</u></em></h5>
            @endif
            @if($icm_nppm != '')
                <p><strong>Pancreatic Neck/Parenchymal Margin:</strong><span>
                @if($icm_nppm == 'Uninvolved by invasive carcinoma and pancreatic high-grade intraepithelial neoplasia')
                {{ $icm_nppm }} @if($icm_nppm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_nppm_distance }}mm)  @endif
                @elseif($icm_nppm == 'Uninvolved by invasive carcinoma')
                {{ $icm_nppm }} @if($icm_nppm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_nppm_distance }}mm)  @endif
                @else
                {{ $icm_nppm }}
                @endif
            ...
            @if($icm_prupm != '')
                ...
                @if($icm_prupm == 'Uninvolved by invasive carcinoma ') {{ $icm_prupm }} @if($icm_prupm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_prupm_distance }}mm)  @endif
                @else
                {{ $icm_prupm }}
                @endif
                </span></p>
            ...
            @if($icm_bdm != '')
                ...
                @if($icm_bdm == 'Uninvolved by invasive carcinoma and pancreatic high-grade intraepithelial neoplasia')
                {{ $icm_bdm }} @if($icm_bdm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_bdm_distance }}mm)  @endif
                @elseif($icm_bdm == 'Uninvolved by invasive carcinoma')
                {{ $icm_bdm }} @if($icm_bdm_distance != '') => (Distance of invasive carcinoma from margin (mm): {{ $icm_bdm_distance }}mm)  @endif
                @else
                {{ $icm_bdm }}
                @endif
            ...
            @endif
            @if($icm_dmj != '')
                <p><strong>Distal Margin (Distal Duodenal or Jejunal): </strong><span>{{ $icm_dmj }}</span></p>
            @endif
            @if($icm_pmom_specify != '')
                <p><strong>Other Margin(s):</strong><span>
                @if($icm_pmom_specify != '') {{ $icm_pmom_specify }}  ({{ $icm_pmom }})   @endif
                </span></p>
            @endif
            @if($treatment_effect != '')
                <p><strong>Treatment Effect: </strong><span>
                @if($treatment_effect == 'Present')
                {{ $treatment_effect }} @if($treatment_effect_pre != '')  {{ $treatment_effect_pre }} @endif
                @elseif($treatment_effect == 'Absent')
                {{ $treatment_effect }} @if($treatment_effect_pre != '')  {{ $treatment_effect_pre }} @endif
                @else {{ $treatment_effect }}  @endif
                </span></p>
            @endif
            ...
            @if($perineural_invasion != '')
                <p><strong>Perineural Invasion: </strong><span> {{ $perineural_invasion }}</span></p>
            @endif
            @if($ln != '')
            ...
            @if($findings_ni != '' || $findings_cp != '' || $findings_ap != '' || $findings_pi != '' || $findings_ot != '' )
                <p><strong>Additional Pathologic Findings:</strong><span>
                @if($findings_ni != '') {{ $findings_ni }}, @endif
                @if($findings_cp != '') {{ $findings_cp }}, @endif
                @if($tumorsite_pb != '') {{ $tumorsite_pb }}, @endif
                @if($findings_ap != '') {{ $findings_ap }}, @endif
                @if($findings_pi != '') {{ $findings_pi }}, @endif
                @if($findings_ot != '') {{ $findings_ot }} (@if($findings_ot_specify != '') {{ $findings_ot_specify }}). @endif @endif
                </span></p>
            @endif
            @if($ancillary_studies != '') <p><strong> Ancillary Studies</strong><br/> {{ $ancillary_studies }} @endif
      ...
      <hr><hr>
        <div class="row">
                <button type="button"  class="btn btn-primary btn-block" onclick="Export2Doc('exportContent', 'Pancreas Exocrine');">Export as .doc</button>
            </div>
        </div>
      </div>
    </section>
