    ...
    <section>
      <div id="exportContent" class="container">
        ...
        <div class="col-md-12">
		     ...
		     @if($procedure != '')<p><strong>Procedure:</strong><span>@if($procedure == 'Other') {{ $procedure }}: {{ $procedure_other }} @else {{ $procedure }}  @endif</span></p>@endif
          @if($specimenl != '')<p><strong>Specimen Laterality:</strong><span> {{ $specimenl }} </span></p>@endif

           @if($tumorsite_upper_outer != '' || $tumorsite_lower_outer != '' || $tumorsite_upper_inner != '' || $tumorsite_lower_inner != '' || $tumorsite_central != '' || $tumorsite_central != '' || $tumorsite_nipple != '' || $tumorsite_notspecified != '' ||  $tumorsite_clock != '' || $tumorsite_distance_fromnipple != '' || $tumorsite_other != '')<p><strong>Tumor Site:</strong>
            <span>
            @if($tumorsite_upper_outer != '') {{ $tumorsite_upper_outer }}, @endif
            @if($tumorsite_lower_outer != '') {{ $tumorsite_lower_outer }}, @endif
            @if($tumorsite_upper_inner != '') {{ $tumorsite_upper_inner }}, @endif
            @if($tumorsite_lower_inner != '') {{ $tumorsite_lower_inner }}, @endif
            @if($tumorsite_central != '') {{ $tumorsite_central }}, @endif
            @if($tumorsite_nipple != '') {{ $tumorsite_nipple }}, @endif
            @if($tumorsite_notspecified != '') {{ $tumorsite_notspecified }}, @endif
            @if($tumorsite_clock != '') {{ $tumorsite_clock }} ({{ $tumorsite_clock_position }} o'clock), @endif
            @if($tumorsite_distance_fromnipple != '') {{ $tumorsite_distance_fromnipple }} ({{ $tumorsite_distance_realdistance }} cm ), @endif
            @if($tumorsite_other != '') {{ $tumorsite_other }} {{ $tumorsite_specify }}, @endif
            </span>
         </p>@endif
         @if($tumorsize != '') <p><strong>Tumor Size:</strong> <span>@if($tumorsize == 'Greatest dimension of largest invasive focus >1 mm:') {{ $tumorsize }} ({{ $tumorsize_gmlength }} mm ),
         + Additional Dimension: {{ $tumorsize_length }} X {{ $tumorsize_width }} mm @elseif($tumorsize == 'Size of largest invasive focus cannot be determined') {{ $tumorsize }}
         Explain Please: {{ $tumorsize_explain }} @endif
         </span></p>@endif
         @if($histologic != '') <p><strong>Histologic Type:</strong><span> @if($histologic == 'Other histologic type not listed') {{ $histologic }}: {{ $histologic_explain }} @else {{ $histologic }} @endif</span></p> @endif
         @if($histologic_grade != '')<p><strong>Histologic Grade (Nottingham Histologic Score):</strong> {{ $histologic_grade }}</p>@endif
         @if($glandular != '')<p><strong>Glandular (Acinar)/Tubular Differentiation:</strong><span> {{ $glandular }}</span></p>@endif
         @if($nuclear_pleomorphism != '')<p><strong>Nuclear Pleomorphism:</strong><span> {{ $nuclear_pleomorphism }}</span></p>@endif
         @if($mitotic_rate != '')<p><strong>Mitotic_rate:</strong><span> {{ $mitotic_rate }}</span></p>@endif

          @if($overall_grade != '')<p><strong>Overall Grade:</strong><span> @if($overall_grade == 'Score cannot be determined') {{ $overall_grade }}: (explain) {{ $overall_gradeexplain }} @else {{ $overall_grade }} @endif</span></p> @endif
          @if($tumor_focality != '')<p><strong>+Tumor Focality:</strong><span> @if($tumor_focality == 'Multiple foci of invasive carcinoma') {{ $tumor_focality }} <br/><span> @if($foci_no != '') Number of foci: {{ $foci_no }} @elseif($foci_atleast != '') >Number of foci is at least: {{ $foci_atleast }} @elseif($foci_undetermined != '' ) {{ $foci_undetermined}} @elseif($foci_size != '') Sizes of individual foci (millimeters): {{ $foci_size }} @endif</span> @else {{ $tumor_focality }} @endif</span></p> @endif
          @if($dcis != '')<p><strong>Ductal Carcinoma In Situ (DCIS):</strong><span> @if($dcis == 'Present') {{ $dcis }} ({{ $dcis_present }}) @else {{ $dcis }} @endif</span></p>@endif
          @if($dcis_size != '')<p>@if($dcis_size != '' || $dcis_block != '')<strong>Size (Extent) of DCIS:</strong><span>@if($dcis_size != '')  Estimated size (extent) of DCIS is at least: {{ $dcis_size }} mm + Additional Dimension (millimeters): {{ $dcis_size_length }} X {{ $dcis_size_width }} mm @endif  @if($dcis_block != '') Number of blocks with DCIS: {{ $dcis_block }} +Number of blocks examined: {{ $dcis_block_examined }} @endif  @endif</span></p>@endif
         @if($dcis != 'Present')
         @if($dcis_ap_comedo != '' || $dcis_ap_pd != '' || $dcis_ap_cribriform != '' || $dcis_ap_micropapillary != '' || $dcis_ap_micropapillary != '' || $dcis_ap_papillary != '' || $dcis_ap_solid != '' || $dcis_ap_other != '') <p><strong>+Architectural Patterns:</strong><span>
              @if($dcis_ap_comedo != '') {{ $dcis_ap_comedo }}, @endif
              @if($dcis_ap_pd != '') {{ $dcis_ap_pd }}, @endif
              @if($dcis_ap_cribriform != '') {{ $dcis_ap_cribriform }}, @endif
              @if($dcis_ap_micropapillary != '') {{ $dcis_ap_micropapillary }}, @endif
              @if($dcis_ap_papillary != '') {{ $dcis_ap_papillary }}, @endif
              @if($dcis_ap_solid != '') {{ $dcis_ap_solid }}, @endif
              @if($dcis_ap_other != '') {{ $dcis_ap_other }} ({{ $dcis_ap_otherspecify }}) @endif
         </span></p>@endif
        @endif
         @if($dcis_nuclear_grade != '')<p><strong>+Nuclear Grade: </strong>{{ $dcis_nuclear_grade }} </span></p>@endif
          @if($dcis_necrosis != '')<p><strong>+Necrosis: </strong><span> {{ $dcis_necrosis }} </span></p>@endif
          @if($lcis != '')<p><strong>+Lobular Carcinoma In Situ (LCIS): </strong><span> {{ $lcis }} </span></p>@endif
          @if($tumor_extension_skin != '' || $tumor_extension_skin != '' || $tumor_extension_nipple != '' || $tumor_extension_sm != '') <h4><strong>Tumor Extension:</strong></h4>@endif
         @if($tumor_extension_skin != '')<p><strong>Skin: </strong><span> {{ $tumor_extension_skin }} </span></p>@endif
         @if($tumor_extension_nipple != '')<p><strong>Nipple: </strong><span> {{ $tumor_extension_nipple }} </span></p>@endif
         @if($tumor_extension_sm != '')<p><strong>Skeletal Muscle: </strong><span> {{ $tumor_extension_sm }} </span></p>@endif
         @if($lcis != '')<h4><strong>Margins</strong></h4>@endif

          @if($icm != '') <p><strong>Invasive Carcinoma Margins:</strong><span> @if($icm = 'Uninvolved by invasive carcinoma') {{ $icm }} @endif
         @if($icm_distance_specify != '')<strong>Distance from closest margin (millimeters):</strong> {{ $icm_distance_specify }}mm @elseif($icm_distance_less_than != '') >Less than {{ $icm_distance_less_than }} mm @elseif($icm_distance_greaterthan != '' ) Greater than {{ $icm_distance_greaterthan}}mm @elseif($icm_distance != '') {{ $icm_distance }}{{ $icm_distance_explain }} @endif

         @if($icm_margins_specify != '') <strong>Specify closest margin: </strong>  {{ $icm_margins_specify }} mm @endif

         @if($icm_margins == 'Cannot be determined') <strong>{{ icm_margins }}:</strong> {{ $icm_margins_explain }} @else {{ $icm_margins }} mm @endif

         <p><strong>Distance from other margins:</strong>
         @if($icm_margins_distance_anterior != '')
         (Anterior: {{ $icm_margins_distance_anterior }}mm), @endif
         @if($icm_margins_distance_posterior != '')
         (Posterior: {{ $icm_margins_distance_posterior }}mm), @endif
         @if($icm_margins_distance_superior != '')
         (Superior: {{ $icm_margins_distance_superior }}mm), @endif
         @if($icm_margins_distance_inferior != '')
         (Inferior: {{ $icm_margins_distance_inferior }}mm), @endif
         @if($icm_margins_distance_medial != '')
         (Medial: {{ $icm_margins_distance_medial }}mm), @endif
         @if($icm_margins_distance_lateral != '')
         (Lateral: {{ $icm_margins_distance_lateral }}mm), @endif
         @if($icm_margins_distance_others != '')
         (Other: {{ $icm_margins_distance_others }}), @endif
        </p>

         @elseif($icm == 'Positive for invasive carcinoma') {{ $icm }}
         @if($pic_anterior != '')
         ({{ $pic_anterior }}: {{ $pic_anterior_ex }}mm), @endif
         @if($pic_posterior != '')
         ({{ $pic_posterior }}: {{ $pic_posterior_ex }}mm), @endif
         @if($pic_superior != '')
         ({{ $pic_superior }}: {{ $pic_superior_ex }}mm), @endif
         @if($pic_inferior != '')
         ({{ $pic_inferior }}: {{ $pic_inferior_ex }}mm), @endif
         @if($pic_medial != '')
         ({{ $pic_medial }}: {{ $pic_medial }}mm), @endif
         @if($pic_lateral != '')
         ({{ $pic_lateral }}: {{ $pic_lateral_ex }}mm), @endif
         @if($pic_others != '')
         ({{ $pic_others }}: {{ $pic_others_ex }}), @endif
         @if($pic_undertermined != '')
         ({{ $pic_undertermined }}: {{ $pic_undetermined_explain }}), @endif
         @else
         {{ $icm }}
         @endif

         @if($dcism != '')<p><strong>DCIS Margins:</strong><span> @if($dcism == 'Uninvolved by DCIS') {{ $dcism }} <br/>
         @if($dcism_distance_specify != '') <strong>Distance from closest margin (millimeters):</strong> @if($dcism_distance_specify != '') {{ $dcism_distance_specify }}mm @elseif($dcism_distance_less_than != '') >Less than {{ $dcism_distance_less_than }} mm @elseif($dcism_distance_greaterthan != '' ) Greater than {{ $dcism_distance_greaterthan}}mm @elseif($dcism_distance != '') {{ $dcism_distance }}{{ $dcism_distance_explain }} @endif @endif
         @if($dcism_margins != '') <strong>Specify closest margin: </strong> @if($dcism_margins == 'Cannot be determined') {{ dcism_margins }}: {{ $dcism_margins_explain }} @else {{ $dcism_margins }}mm @endif @endif
         <br/>
         <strong>Distance from other margins:</strong>
         @if($dcism_margins_distance_anterior != '')
         (Anterior: {{ $dcism_margins_distance_anterior }}mm), @endif
         @if($dcism_margins_distance_posterior != '')
         (Posterior:  {{ $dcism_margins_distance_posterior }}mm), @endif
         @if($dcism_margins_distance_superior != '')
         (Superior:  {{ $dcism_margins_distance_superior }}mm), @endif
         @if($dcism_margins_distance_inferior != '')
         (Inferior:  {{ $dcism_margins_distance_inferior }}mm), @endif
         @if($dcism_margins_distance_medial != '')
         (Medial:  {{ $dcism_margins_distance_medial }}mm), @endif
         @if($dcism_margins_distance_lateral != '')
         (Lateral: {{ $dcism_margins_distance_lateral }}mm), @endif
         @if($dcism_margins_distance_others != '')
         (Other:  {{ $dcism_margins_distance_others }}), @endif
         @endif

         @elseif($dcism == 'Positive for invasive carcinoma') {{ $dcism }}
         @if($pdcis_anterior != '')
         ({{ $pdcis_anterior }}: {{ $pdcis_anterior_ex }}mm), @endif
         @if($pdcis_posterior != '')
         ({{ $pdcis_posterior }}: {{ $pdcis_posterior_ex }}mm), @endif
         @if($pdcis_superior != '')
         ({{ $pdcis_superior }}:  {{ $pdcis_superior_ex }}mm), @endif
         @if($pdcis_inferior != '')
         ({{ $pdcis_inferior }}:  {{ $pdcis_inferior_ex }}mm), @endif
         @if($pdcis_medial != '')
         ({{ $pdcis_medial }}:  {{ $pdcis_medial_ex }}mm), @endif
         @if($pdcis_lateral != '')
         ({{ $pdcis_lateral }}: {{ $pdcis_lateral_ex }}mm), @endif
         @if($pdcis_others != '')
         ({{ $pdcis_others }}:  {{ $pdcis_others_ex }}), @endif
         @if($pdcis_undertermined != '')
         ({{ $pdcis_undertermined }}: {{ $pdcis_undetermined_explain }}), @endif
         @else
         {{ $dcism }}
         @endif
        </span><br/></p>
        @if($rln != '')<p><strong>Regional Lymph Nodes:</strong><span> @if($rln == 'Uninvolved by tumor cells') {{ $rln }}<br/>
        @if($rln_uninvolved_ln != '') <strong>Number of Lymph Nodes Examined:</strong> {{ $rln_uninvolved_ln }} <br/> @endif
        @if($rln_uninvolved_sn != '') <strong>Number of Sentinel Nodes Examined:</strong> {{ $rln_uninvolved_sn }} @endif
        @elseif($rln == 'Involved by tumor cells')<br/>
        @if($rln_involved_ln_mac != '') <strong>Number of Lymph Nodes with Macrometastases:</strong> {{ $rln_involved_ln_mac }} <br/> @endif
        @if($rln_involved_ln_mic != '') <strong>Number of Lymph Nodes with Micrometastases:</strong> {{ $rln_involved_ln_mic }} <br/> @endif
        @if($rln_involved_ln_itc != '') <strong>Number of Lymph Nodes with Isolated Tumor Cells:</strong> {{ $rln_involved_ln_itc }} <br/>@endif
        @if($rln_deposit != '') <strong>Size of Largest Metastatic Deposit:</strong> {{ $rln_deposit }} <br/> @endif
        @if($extranodal_ext != '') <strong>Extranodal Extension:</strong> {{ $extranodal_ext }} <br/> @endif
        @if($ln_examined != '') <strong>Number of Lymph Nodes Examined:</strong> {{ $ln_examined }} <br/> @endif
        @if($sn_examined != '') <strong>Number of Sentinel Nodes Examined:</strong> {{ $sn_examined }} @endif
        @else
        {{ $rln }}
        @endif
        </span></p>@endif
         @if($presurgical_therapy != '')<p><strong>Treatment Effect:</strong><span> {{ $presurgical_therapy }}</span></p>@endif
        @if($presurgical_therapy == 'Known presurgical therapy')
        <p><strong>Treatment Effect in the Breast:</strong><span> {{ $breast_treatment_effect }} </span></p>
        <p><strong>Treatment Effect in the Lymph Nodes:</strong><span> {{ $ln_treatment_effect }} </span></p>
        <p><strong>Lymphovascular Invasion:</strong><span> {{ $lymphovascular_invasion }} </span></p>
        <p><strong>Dermal Lymphovascular Invasion:</strong><span> {{ $dermal_lymphovascular_invasion }} </span></p>
        @endif
        ...
        @if($distance_metastasis != '')<p><strong>Distance Metastasis: </strong><span> @if($distance_metastasis == 'pM1:  Histologically proven metastases larger than 0.2 mm') {{ $distance_metastasis}} @if($distance_metastasis_site != '') Specify site: {{ $distance_metastasis_site }} @endif @else {{ $distance_metastasis }} @endif </span></p>@endif

        @if($modifier != '' || $category != '' || $additional_finding != '' || $ancillary_studies != '') <p><strong>Regional Lymph Nodes (pN) (choose a category based on lymph nodes received with the specimen; immunohistochemistry and/or molecular studies are not required)</strong><br/> <i>Note: If internal mammary lymph nodes, infraclavicular lymph nodes, or supraclavicular lymph nodes are included in the specimen, consult the AJCC Staging Manual for additional lymph node categories</i></p> @endif
        @if($modifier != '')<p><strong>Modifier:</strong><span> {{ $modifier }}</span></p>@endif
        @if($category != '')<p><strong>Category (pN):</strong><span> {{ $category }}</span></p>@endif
        @if($additional_finding != '')<p><strong>{{ $additional_finding }}</strong><span> {{ $additional_finding_note }}</span></p>@endif
        @if($ancillary_studies != '')<p><strong>{{ $ancillary_studies }}</strong><span><br/> <i>Note: For hormone receptor and HER2 reporting for this specimen, the CAP Breast Biomarker Template should be used. Pending biomarker studies should be listed in the Comments section of this report.</i></span></p>@endif
        @if($estrogen_receptor != '')<p><strong>+ Breast Biomarker Testing Performed on Previous Biopsy</strong></p>@endif
        @if($estrogen_receptor != '')<p><strong>+ Estrogen Receptor (ER):</strong><span> @if($estrogen_receptor == 'Positive') {{ $estrogen_receptor }}: {{ $estrogen_receptor_percent }} @else {{ $estrogen_receptor  }} @endif</span></p>    @endif
        @if($estrogen_receptor != '')<p><strong>+ Progesterone Receptor (PgR):</strong><span> @if($estrogen_receptor == 'Positive') {{ $progesterone_receptor }}: {{ $progesterone_receptor_percent }} @else {{ $progesterone_receptor  }} @endif</span></p>@endif
        @if($her2 != '')<p><strong>+ HER2 (by immunohistochemistry):</strong><span> {{ $her2  }} </span></p>@endif
        @if($her2_situ != '')<p><strong>+ HER2 (by in situ hybridization):</strong><span> {{ $her2_situ  }} </span></p>@endif
        @if($case_number != '')<p><strong>+Testing Performed on Case Number:</strong><span> {{ $case_number  }} </span></p>@endif

        @if($microcalcifications_notidentified != '' || $microcalcifications_present != '' || $microcalcifications_presentic != '' || $microcalcifications_presentnnt != '' || $microcalcifications_other != '')<p><strong>Microcalcifications:</strong>
        <span>
            @if($microcalcifications_notidentified != '') {{ $microcalcifications_notidentified }}, @endif
            @if($microcalcifications_present != '') {{ $microcalcifications_present }}, @endif
            @if($microcalcifications_presentic != '') {{ $microcalcifications_presentic }}, @endif
            @if($microcalcifications_presentnnt != '') {{ $microcalcifications_presentnnt }}, @endif
            @if($microcalcifications_other != '') {{ $microcalcifications_other }}: {{ $microcalcifications_other_specify }}, @endif

        </span></p>@endif
         @if($breast_finding_pm != '' || $breast_finding_nd != '' || $breast_finding_o != '')<p><strong>Clinical History: <i>The current clinical/radiologic breast findings for which this surgery is performed include:</i></strong>@endif
        <span>
        @if($breast_finding_pm != '' || $breast_finding_nd != '' || $breast_finding_o != '') @if($breast_finding_o == 'Other'){{ $breast_finding_o }}: {{ $breast_finding_specify }}, @endif @if($breast_finding_pm != '') {{ $breast_finding_pm }}, @endif @if($breast_finding_nd != '') {{ $breast_finding_nd }}, @endif @endif
        @if($breast_history != '') @if($breast_finding == 'Prior history of breast cancer'){{ $breast_history }}, {{ $breast_history_finding }}, @endif @endif
        @if($breast_prior_tx != '') {{ $breast_prior_tx }} @endif
        </span></p>
         @if($radiologic_finding_mass != '' || $radiologic_finding_calc != '' || $radiologic_finding_other != '')<p><strong>Radiologic Finding:</strong>@endif
        <span>,
        @if($radiologic_finding_mass != '' || $radiologic_finding_calc != '' || $radiologic_finding_other != '') @if($radiologic_finding_other != ''){{ $radiologic_finding_other }}: {{ $radiologic_finding_specify }} @endif @if($radiologic_finding_calc != '') {{ $radiologic_finding_calc }} @endif @if($radiologic_finding_mass != '') {{ $radiologic_finding_mass }}@endif @endif
        </span></p>

          @if(($primary_tumor == 'pTX:	Primary tumor cannot be assessed' || $primary_tumor == 'pTis (DCIS):	Ductal carcinoma in situ#' ||  $primary_tumor == 'pTis (Paget):	Paget disease of the nipple not associated with invasive carcinoma and/or DCIS in the underlying breast parenchyma##') &&  ($category == 'pNX:	Regional lymph nodes cannot be assessed (eg, not removed for pathological study or previously removed)' || $category == 'pN0:	No regional lymph node metastasis identified or ITCs only#' || $category == 'pN0 (i+):	ITCs only (malignant cell clusters no larger than 0.2 mm) in regional lymph node(s)' || $category == 'pN0 (mol+):	Positive molecular findings by reverse transcriptase polymerase chain reaction (RT PCR); no ITCs detected') && ($distance_metastasis == 'pM0: No proven metastasis'))
            <p><strong>Group Stage:</strong><span> 0</span></p>
          @elseif(($primary_tumor == 'pT1mi: Tumor ≤1 mm in greatest dimension' || $primary_tumor == 'pT1a:	Tumor >1 mm but ≤5 mm in greatest dimension (round any measurement >1.0−1.9 mm to 2 mm)' || $primary_tumor == 'pT1b:	Tumor >5 mm but ≤10 mm in greatest dimension' || $primary_tumor == 'pT1c:	Tumor >10 mm but ≤20 mm in greatest dimension') && ($category == 'pNX:	Regional lymph nodes cannot be assessed (eg, not removed for pathological study or previously removed)' || $category == 'pN0:	No regional lymph node metastasis identified or ITCs only#' || $category == 'pN0 (i+):	ITCs only (malignant cell clusters no larger than 0.2 mm) in regional lymph node(s)' || $category == 'pN0 (mol+):	Positive molecular findings by reverse transcriptase polymerase chain reaction (RT PCR); no ITCs detected') && ($distance_metastasis == 'pM0: No proven metastasis'))
            <p><strong>Group Stage:</strong><span> IA</span></p>
          @elseif(($primary_tumor == 'pT0:	No evidence of primary tumor#') && ($category == 'pN1mi:	Micrometastases (approximately 200 cells, larger than 0.2 mm, but none larger than 2.0 mm)') &&  ($distance_metastasis == 'pM0: No proven metastasis'))
          ...
          @elseif(($primary_tumor == 'pT1mi: Tumor ≤1 mm in greatest dimension' || $primary_tumor == 'pT1a:	Tumor >1 mm but ≤5 mm in greatest dimension (round any measurement >1.0−1.9 mm to 2 mm)' || $primary_tumor == 'pT1b:	Tumor >5 mm but ≤10 mm in greatest dimension' || $primary_tumor == 'pT1c:	Tumor >10 mm but ≤20 mm in greatest dimension') && ($category == 'pN1mi:	Micrometastases (approximately 200 cells, larger than 0.2 mm, but none larger than 2.0 mm)') &&  ($distance_metastasis == 'pM0: No proven metastasis'))
            <p><strong>Group Stage:</strong><span> IB</span></p>
          @elseif(($primary_tumor == 'pT0:	No evidence of primary tumor#') && ($category == 'pN1mi:	Micrometastases (approximately 200 cells, larger than 0.2 mm, but none larger than 2.0 mm)' || $category == 'pN1a:	Metastases in 1 to 3 axillary lymph nodes, at least 1 metastasis larger than 2.0 mm##' || $category == 'pN1b:	Metastases in ipsilateral internal mammary sentinel nodes, excluding ITCs' || $category == 'pN1c:	pN1a and pN1b combined') &&  ($distance_metastasis == 'pM0: No proven metastasis'))
          ...
          @elseif(($primary_tumor == 'pT3:	Tumor >50 mm in greatest dimension')  && ($category == 'pN2a:	Metastases in 4 to 9 axillary lymph nodes (at least 1 tumor deposit larger than 2.0 mm)##' || $category == 'N2b:	Metastases in clinically detected internal mammary lymph nodes with or without microscopic confirmation; with pathologically negative axillary nodes')  &&  ($distance_metastasis == 'pM0: No proven metastasis'))
            <p><strong>Group Stage:</strong><span> IIIA</span></p>
          @elseif(($primary_tumor == 'pT4:  Tumor of any size with direct extension to the chest wall and/or to the skin (ulceration or skin nodules) ###' || $primary_tumor == 'pT4a:	Extension to the chest wall; invasion or adherence to pectoralis muscle in the absence of invasion of chest wall structures does not qualify as T4' || $primary_tumor == 'pT4b:	Ulceration and/or ipsilateral macroscopic satellite nodules and/or edema (including peau d’orange) of the skin that does not meet the criteria for inflammatory carcinoma' || $primary_tumor == 'pT4c:	Both T4a and T4b are present' || $primary_tumor == 'pT4d:	Inflammatory carcinoma####' ) && ($category == 'pNX:	Regional lymph nodes cannot be assessed (eg, not removed for pathological study or previously removed)' || $category == 'pN0:	No regional lymph node metastasis identified or ITCs only#' || $category == 'pN0 (i+):	ITCs only (malignant cell clusters no larger than 0.2 mm) in regional lymph node(s)' || $category == 'pN0 (mol+):	Positive molecular findings by reverse transcriptase polymerase chain reaction (RT PCR); no ITCs detected') && ($distance_metastasis == 'pM0: No proven metastasis'))
