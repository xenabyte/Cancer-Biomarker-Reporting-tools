  ...
  <div class="main-content">
    ...
    <section>
      <div id="exportContent" class="container">
        ...
        <div class="col-md-12">
            @if($procedure_excision != '' || $procedure_paro_super != '' || $procedure_paro_deep != '' || $procedure_paro_total != '' || $procedure_paro_ns != '' || $procedure_res_subman != '' || $procedure_res_sublin != '' || $procedure_neck_desection != '' || $procedure_other != '')
                <p><strong>Procedure:</strong><span>@if($procedure_excision != '') {{ $procedure_excision }}, @endif @if($procedure_paro_super != '') {{ $procedure_paro_super }}, @endif @if($procedure_paro_deep != '') {{ $procedure_paro_deep }}, @endif  @if($procedure_paro_total != '') {{ $procedure_paro_total }}, @endif @if($procedure_paro_ns != '') {{ $procedure_paro_ns }}, @endif  @if($procedure_res_subman != '') {{ $procedure_res_subman }}, @endif @if($procedure_res_sublin != '') {{ $procedure_res_sublin }}, @endif @if($procedure_not_specified != '') {{ $procedure_not_specified }} ,@endif @if($procedure_neck_desection != '')  Neck (lymph node) dissection (specify): {{ $procedure_neck_desection }}, @endif @if($procedure_other != '') Other (specify): {{ $procedure_other }} @endif</span></p>@endif
            @if($tumorsite != '')
                <p><strong>Tumor Site:</strong><span>@if($tumorsite == 'Parotid gland') {{ $tumorsite }} ({{ $patotid_gland_option }}) @elseif($tumorsite_other != '')  Other (specify): {{ $tumorsite_other }} @else {{ $tumorsite }} @endif</span></p>
            @endif
            @if($tumorliterality != '')
                <p><strong>Tumor Laterality:</strong> {{ $tumorliterality }} <span></span></p>
            @endif
            @if($tumorfocality != '')
                <p><strong>Tumor Focality:</strong> {{ $tumorfocality }} <span></span></p>
            @endif
            @if($tumorsize_gt != '' || $tumorsize_cd != '')
                <p><strong>Tumor Size:</strong>@if($tumorsize_cd != '') Cannot be determined (explain): {{ $tumorsize_cd }} @else  Greatest dimension (centimeters): {{ $tumorsize_gt }}cm @if($tumorsize_length != '' && $tumorsize_width != '') => Additional dimensions (centimeters): {{ $tumorsize_length }}cm X {{ $tumorsize_width }}cm  @endif  @endif<span></span></p>
            @endif
            @if($histologic != '')
                <p><strong>Histologic Type:</strong>@if($histologic == 'Adenoid cystic carcinoma, tubular pattern' || $histologic == 'Adenoid cystic carcinoma, cribriform pattern' || $histologic == 'Adenoid cystic carcinoma, solid pattern') {{ $histologic }} (Specify percentage of solid component: {{ $histologic_cent }}% ) @elseif($histologic == 'Other histologic type not listed (specify):') {{ $histologic }}: {{ $histologic_explain }} @else  {{ $histologic }}  @endif<span></span></p>
            @endif
            @if($transformation != '')
                <p><strong>High-Grade Transformation:</strong> {{ $transformation }} <span></span></p>
            @endif
            @if($tumor_extension != '')
                <p><strong>Tumor Extention:</strong> {{ $tumor_extension }} <span></span></p>
            @endif
            @if($icm != '')
                <p><strong>Margins:</strong><span>@if($icm == 'Involved by invasive carcinoma') {{ $icm }}  @if($icm_involve_distance_specify != '') Specify margin, if possible:  {{ $icm_involve_distance_specify }}mm @endif @else {{ $icm }} @if($icm_closest_margin != '') Distance of tumor from closest margin (millimeters): {{ $icm_closest_margin }}mm @endif @if($icm_distance_specify != '') Specify margin, if possible:  {{ $icm_distance_specify }}mm  @endif @endif </span></p>
            @endif
            ...
            @if($ancillary_studies != '')
                <p><strong>Additional Pathologic Findings:</strong><span>@if($ancillary_studies == 'Other') {{ $ancillary_studies }} ({{ $ancillary_studies_others }}) @else {{ $ancillary_studies }} @endif </span></p>
            @endif
